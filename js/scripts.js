...
async function loadHistorico() {
    ...
    pedidos.forEach(pedido => {
        ...
        row.innerHTML = `
            <td class="historico-nome" data-pedido-id="${pedido.id}" data-creator-id="${pedido.user_id}">${pedido.titulo}</td>
            <td>${pedido.categoria}</td>
            <td>${new Date(pedido.created_at).toLocaleDateString('pt-BR')}</td>
            <td>${pedido.status}</td>
            <td><span class="historico-expand-btn">▼</span></td>
        `;
        ...
        expansionDiv.classList.add('historico-expansion');
        expandTd.appendChild(expansionDiv);
        expandRow.appendChild(expandTd);
        tableBody.appendChild(expandRow);

        // Preenche expansão baseada no status
        if (pedido.status === 'Disponível') {
            expansionDiv.innerHTML = `
                <button class="historico-delete-btn">Excluir Pedido</button>
            `;
            expansionDiv.querySelector('.historico-delete-btn').addEventListener('click', async () => {
                if (confirm('Exclusão permanente. Confirmar?')) {
                    const { error } = await supabaseClient.from('pedidos').delete().eq('id', pedido.id);
                    if (!error) loadHistorico(); // Recarrega
                }
            });
        } else if (pedido.status === 'Pendente') {
            expansionDiv.innerHTML = `
                <div class="historico-tracking">
                    <input type="text" value="${pedido.tracking_code || ''}" readonly>
                    <button class="historico-copy-btn">Copiar</button>
                </div>
                <div class="historico-checkboxes">
                    <label><input type="checkbox" name="opcao" value="invalido"> Código Inválido</label>
                    <label><input type="checkbox" name="opcao" value="entregue"> Produto Entregue</label>
                </div>
                <p class="historico-note">Conferir o código com atenção.</p>
                <button class="historico-confirm-btn">Confirmar</button>
            `;
            const checkboxes = expansionDiv.querySelectorAll('input[type="checkbox"]');
            checkboxes.forEach(cb => {
                cb.addEventListener('change', () => {
                    if (cb.checked) checkboxes.forEach(other => { if (other !== cb) other.checked = false; });
                });
            });
            expansionDiv.querySelector('.historico-copy-btn').addEventListener('click', () => {
                navigator.clipboard.writeText(pedido.tracking_code);
            });
            expansionDiv.querySelector('.historico-confirm-btn').addEventListener('click', async () => {
                const selected = expansionDiv.querySelector('input[name="opcao"]:checked');
                if (!selected) {
                    alert('Selecione uma opção');
                    return;
                }
                let newStatus = selected.value === 'invalido' ? 'Disponível' : 'Concluído';
                let updates = { status: newStatus };
                if (newStatus === 'Concluído') updates.completion_date = new Date().toISOString();
                const { error } = await supabaseClient.from('pedidos').update(updates).eq('id', pedido.id);
                if (!error) loadHistorico();
            });
        } else if (pedido.status === 'Concluído') {
            expansionDiv.innerHTML = `
                <p class="historico-completion-date">Finalizado em: ${new Date(pedido.completion_date).toLocaleDateString('pt-BR')}</p>
            `;
        }
    });
}
...
document.addEventListener('click', (e) => {
    if (e.target.classList.contains('historico-expand-btn')) {
        const expansion = e.target.closest('tr').nextElementSibling.querySelector('.historico-expansion');
        expansion.classList.toggle('show');
        e.target.textContent = expansion.classList.contains('show') ? '▲' : '▼';
    } else if (e.target.classList.contains('historico-nome')) {
        const pedidoId = e.target.dataset.pedidoId;
        const creatorId = e.target.dataset.creatorId;
        window.location.href = `perfil.html?user=${creatorId}&pedido=${pedidoId}`;
    }
});

// Carrega ao iniciar (adicione em window.onload ou DOMContentLoaded)
