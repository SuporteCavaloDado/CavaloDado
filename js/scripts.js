...
function preencherDadosUsuario() {
    ...
    // Funcionabilidade Modal Pedidos Detalhes


// Evento de clique nos títulos
document.querySelectorAll('.pedido-titulo').forEach(titulo => {
    titulo.addEventListener('click', async (e) => {
        const pedidoId = e.target.closest('.pedido-item').dataset.id;
        // Fetch dados de Supabase
        const { data, error } = await supabase.from('pedidos').select('*').eq('id', pedidoId).single();
        if (error) console.error(error);
        else {
            document.getElementById('pedido-foto').src = data.foto_url; // Assuma campo 'foto_url'
            document.getElementById('pedido-nome').textContent = data.titulo;
            document.getElementById('pedido-descricao').textContent = data.descricao;
            document.getElementById('pedido-categoria').textContent = data.categoria;
            document.getElementById('pedido-estado').textContent = data.estado; // Assuma campo
            document.getElementById('pedido-status').textContent = data.status;
            document.getElementById('pedido-data').textContent = new Date(data.created_at).toLocaleDateString();
...
// Fechar modal
document.querySelector('.modal-pedido-close').addEventListener('click', () => {
...
document.querySelector('.btn-doar').addEventListener('click', () => {
    window.location.href = 'doacao.html?pedido=' + currentPedidoId; // Assuma ID
});
...
document.querySelector('.btn-favorito').addEventListener('click', async () => {
    ...
    const table = isFavorito ? 'favoritos' : ''; // Insira/remova em Supabase
    // Ex: await supabase.from('favoritos').upsert({ user_id: user.id, pedido_id: currentPedidoId });
    this.textContent = isFavorito ? 'Desfavoritar' : 'Favorito';
});

// Botão Compartilhar
document.querySelector('.btn-compartilhar').addEventListener('click', () => {
    navigator.share({ url: window.location.href + '?pedido=' + currentPedidoId });
});
